...
export function calculateResult(answers: Answers) {
  ...
  const ruleClarity = avg(
    pick([15, 40, 70, 90], q(2)),
    pick([15, 35, 65, 90], q(4)),
    pick([10, 30, 55, 80, 95], q(17)),
    pick([10, 30, 55, 80, 95], q(21))
  );
  const timeAvailability = avg(pick([10, 35, 70, 95], q(5)), pick([10, 35, 70, 95], q(6)));
  const riskControl = avg(
    pick([10, 30, 55, 80, 95], q(17)),
    pick([35, 25, 10, 85, 75], q(18)),
    pick([10, 15, 5, 45, 85], q(19)),
    pick([10, 30, 55, 80, 95], q(21)),
    pick([10, 15, 30, 75, 90], q(22))
  );
  ...
  let scalpingFit = avg(
    timeAvailability,
    pick([90, 60, 20, 5], q(7)),
    pick([10, 30, 55, 90], q(8)),
    pick([95, 70, 25, 10], q(9))
  );
  if (timeAvailability < 45) scalpingFit = Math.min(scalpingFit, 45);
  if (riskControl < 45) scalpingFit = Math.min(scalpingFit, 50);

  const swingFit = avg(
    pick([40, 75, 95, 55], q(7)),
    pick([25, 70, 85, 30], q(8)),
    pick([20, 70, 95, 65], q(9)),
    timeAvailability < 70 ? 80 : 60
  );
  const trendFit = avg(
    pick([10, 40, 85, 95], q(7)),
    pick([70, 60, 45, 20], q(8)),
    pick([10, 45, 85, 95], q(9)),
    currentLevel
  );
  const pullbackFit = avg(
    q(10) === 3 ? 90 : 40,
    q(13) === 4 ? 85 : q(13) === 3 ? 70 : 35,
    q(16) === 3 ? 90 : 40,
    q(15) === 4 ? 75 : q(15) === 5 ? 85 : 35,
    pick([20, 55, 90, 60], q(7))
  );
  ...
  const typeScores: Record<ResultTypeKey, number> = {
  ...
  const sortedTypes = Object.entries(typeScores).sort((a, b) => b[1] - a[1]) as [ResultTypeKey, number][];
  ...
  const recommendedTimeframe =
    mainType === "scalpingSprinter" && timeAvailability >= 65 && riskControl >= 60
      ? "5분봉 + 15분봉"
      : mainType === "swingMaker" || swingFit >= 70
        ? "일봉 + 4시간봉"
        : mainType === "trendNavigator" || trendFit >= 70
          ? "일봉 + 주봉"
          : mainType === "boxStrategist"
            ? "1시간봉 + 4시간봉"
            : timeAvailability < 45
              ? "일봉 + 4시간봉"
              : "4시간봉 + 일봉";
